...
4. Read `~/work/laniameda/laniameda-hq/studio/identity.md` + `~/work/laniameda/laniameda-hq/work/projects.md` — **studio ground truth** (laniameda-hq is the single source of truth; studio.md is legacy)
...
- Code output → work in the relevant product repo, never auto-deploy
...
## Sprint Workflow (RunMusic + other product sprints)

When we do a focused sprint (e.g. Wrapped redesign):

1. Create a sprint folder in the **product repo**:
   - `pm/sprints/<sprint-name>/` (todo.md + PRDs live here)
2. Create a branch that mirrors the sprint name:
   - `sprint/<sprint-name>` (preferred) or `<sprint-name>`
3. Build **modularly**:
   - scaffold/layout → commit
   - component/module → commit
   - polish/QA → commit
4. Keep a clean paper trail:
   - todo.md stays current
   - each PRD states goal/non-goals + acceptance criteria
5. Merge only after:
   - typecheck/build passes
   - no “pace” UI regressions (RunMusic rule)
