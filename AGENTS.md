# AGENTS.md — Laniameda Studio Context

> **Read this first.** Full context for working with Michael and the Laniameda studio. For any AI assistant — Codex, Codex, or similar.
>
...
git pull
./install-skills.sh              # sync all skills to ~/.agents/skills/ + ~/.Codex/skills/
./install-skills.sh supadata     # sync a single skill
```

**How it works:** Skills are organized in `skills/<category>/<skill>/` folders. `npx skills` can't discover nested dirs, so `install-skills.sh` copies them directly to `~/.agents/skills/` and creates symlinks in `~/.Codex/skills/`. No `npx skills` needed for laniameda skills.
...
|---|---|---|
| Agent orchestration | Specific workflow architectures, "if X then Y" tool mappings, MCP names, Codex patterns | "Agents are powerful" |
...
- **Keep AGENTS.md updated.** When Michael says "remember this" or when new info contradicts this file — update it immediately and commit.
