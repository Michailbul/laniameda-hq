...
| `andromeda-messages` | utility | CRUD for Andromeda galaxy page (mishabuloichyk.com) - add/edit/delete/list nodes, lock/unlock galaxies |
...
## Skill Source of Truth — Non-Negotiable Rule

**All laniameda skills have ONE canonical source: `laniameda-skills` in the `laniameda-hq` repo.**
- Canonical path: `~/work/laniameda/laniameda-hq/laniameda-skills/skills/<category>/<skill-name>/SKILL.md`
- The workspace runtime copy at `/root/.openclaw/workspace-crea/skills/` is a **local convenience copy only**
- It is NOT the source of truth
- It is NOT tracked in git
- It MUST NOT drift from the canonical version

**The only exception: `laniameda-gallery-ingest`**
- Its canonical source is the gallery repo: https://github.com/Michailbul/laniameda-gallery
- Installed via `bun run skills:update`

**Workflow for skill edits:**
1. Edit the canonical skill in `laniameda-skills` first
2. Commit + push that repo
3. Update `laniameda-hq` submodule pointer if needed
4. Optionally copy/sync the runtime local skill if it exists — but canonical = `laniameda-skills`

**Never:**
- Edit the local workspace skill as the primary change
- Assume the local skill and canonical skill are in sync without checking
- Commit AGENTS.md/CLAUDE.md changes bundled with unrelated skill diffs

## Lessons Learned

- Don't fragment early. Centralized master skill + strict use-case templates keeps quality and maintenance stable.
- Skills in `/root/.openclaw/workspace-crea/skills/` are runtime convenience copies, not sources of truth. Always edit `laniameda-skills` first.
- When learning from external content (video, link, post): extract and present first, do NOT save to knowledge/memory/skills until Michael explicitly approves.

## Carousel Design — Validated Rules (2026-04-03)

**What Michael rejects immediately (don't repeat):**
- Black/dark containers or card overlays over images
- Corner L-bracket decorative accents
- Charcoal Violet `#3D2E42` or Slate Blue `#4A5E7A` as backgrounds — off-brand
- Ghost text for attribution/watermarks (opacity 0.2) — must be readable
- Side-by-side images at half-width — always one image per slide, full 1080px
- Leftover divider lines from previous layouts
- Background images on CTA slides

**Typography contrast issues — how they were caused and fixed:**
- Eyebrow too small (12px) on complex bg → lost. Fix: 28–36px DG Bold or 14px+ JB Mono bold, minimum 0.55 opacity
- Sub text too light (opacity 0.22–0.38) → disappeared. Fix: 0.55+ opacity + text-shadow on all image-layer text
- Title not centered on hero → top-anchored. Fix: absolute center via transform(-50%,-50%)
- Bottom chrome text adapted to light bg → disappeared over image. Fix: always light, always over dark image gradient

**Marketing rules for Slide 1:**
- One clear hierarchy: EYEBROW (big, readable) → TITLE (dominant) → sub (supporting)
- Center of image must be darkened for text — radial vignette + linear fades + text-shadow
- Hook has to read as a standalone in feed thumbnail

**CTA slide rule:**
- Use the exact copy Michael writes for the CTA — never paraphrase or genericize
- Attribution must be prominent type, not ghost footer
- Solid background only, no images, no leftover elements
